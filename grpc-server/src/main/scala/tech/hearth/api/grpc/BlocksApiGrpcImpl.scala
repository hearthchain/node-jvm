...
import tech.hearth.api.grpc.BlockRangeRequest.Filter
import tech.hearth.api.grpc.BlockRequest.Request
import tech.hearth.api.http.ApiError.BlockDoesNotExist
import tech.hearth.protobuf.*
import tech.hearth.protobuf.block.PBBlock
...
object BlocksApiGrpcImpl {
  private def toBlockWithHeight(v: (BlockMeta, Seq[(TxMeta, Transaction)])) = {
    ...
    BlockWithHeight(
      ...
      blockMeta.rewardShares.map { case (addr, reward) => RewardShare(addr.toByteString, reward) }
    )
  }
  ...
  private def toBlockWithHeight(m: BlockMeta) =
    BlockWithHeight(
      ...
      m.rewardShares.map { case (addr, reward) => RewardShare(addr.toByteString, reward) }
    )
}
