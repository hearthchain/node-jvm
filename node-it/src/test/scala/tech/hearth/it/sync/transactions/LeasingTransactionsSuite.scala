...
import tech.hearth.it.transactions.BaseTransactionSuiteLike
...
class LeasingTransactionsSuite extends AnyFreeSpecLike, BaseTransactionSuiteLike, CancelAfterFailure {
  ...
  "leasing hearth decreases lessor's eff.b. and increases lessee's eff.b.; lessor pays fee" in {
  ...
  "cannot lease non-own hearth" in {
    ...
    assertBadRequestAndResponse(sender.lease(secondKeyPair, thirdAddress, eff2 - minFee, leasingFee = minFee), errorMessage)
  }

  "can not make leasing without having enough balance" in {
    val (balance1, eff1) = miner.accountBalances(firstAddress)
    val (balance2, eff2) = miner.accountBalances(secondAddress)

    // secondAddress effective balance more than general balance
    assertBadRequestAndResponse(sender.lease(secondKeyPair, firstAddress, balance2 + 1.hearth, minFee), errorMessage)
    nodes.waitForHeightArise()

    assertBadRequestAndResponse(sender.lease(firstKeyPair, secondAddress, balance1, minFee), errorMessage)
    nodes.waitForHeightArise()

    assertBadRequestAndResponse(sender.lease(firstKeyPair, secondAddress, balance1 - minFee / 2, minFee), errorMessage)
    nodes.waitForHeightArise()

    val newAddress = TxHelpers.signer(1000)
    sender.transfer(sender.keyPair, newAddress.toAddress.toString, minFee, minFee, waitForTx = true)
    assertBadRequestAndResponse(sender.lease(newAddress, secondAddress, minFee + 1, minFee), errorMessage)
    nodes.waitForHeightArise()
  ...
  "lease cancellation reverts eff.b. changes; lessor pays fee for both lease and cancellation" in {
    ...
    val createdLeaseTxId = sender.lease(firstKeyPair, secondAddress, leasingAmount, minFee).id
    nodes.waitForHeightAriseAndTxPresent(createdLeaseTxId)
  ...
  "lease cancellation can be done only once" in {
    val (balance1, eff1) = miner.accountBalances(firstAddress)
    val (balance2, eff2) = miner.accountBalances(secondAddress)

    val createdLeasingTxId = sender.lease(firstKeyPair, secondAddress, leasingAmount, minFee).id
  ...
  "only sender can cancel lease transaction" in {
    val (balance1, eff1) = miner.accountBalances(firstAddress)
    val (balance2, eff2) = miner.accountBalances(secondAddress)
  ...
  "can not make leasing to yourself" in {
    val (balance1, eff1) = miner.accountBalances(firstAddress)
    assertBadRequestAndResponse(sender.lease(firstKeyPair, firstAddress, balance1 + 1.hearth, minFee), "Transaction to yourself")
    nodes.waitForHeightArise()
