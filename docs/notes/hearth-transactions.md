...
`sign-with-sk` takes the signing key directly instead of from a config, in either of the two forms one exists in: `--private-key` is the 32-byte Ed25519 seed (what `hearth.miner.accounts` holds as `signing-key-seed`) and `--private-key-scalar` the bare secret scalar (`signing-key-scalar`, what a vanity grinder emits, see "Keys" in `docs/notes/keys-and-signatures.md`). The scalar option exists because a grinder's key has no seed at all, so before it the only way to sign with such a key outside a running node was to write Scala against `SigningKey.fromScalar`. The two are deliberately separate options and a command line naming both is rejected (`Command.signerOptions` counts them, `checkConfig` fails on more than one): the same 32 bytes are a valid seed *and* a valid scalar, both parse, and the two readings sign as different accounts, so last-wins would silently produce a signed transaction from an account the caller did not mean. Nothing here consults a chain, so the `sender` in the output is the only check the caller gets. `docker/README.md`'s "Signing a transaction offline" documents all three operator-facing forms (scalar, seed, mnemonic via `crypto create-keys`), including what a `docker run --entrypoint java` has to pass that `entrypoint.sh` otherwise would.

A container started only to sign has no wallet, which is why `transaction sign --signer-address` is not an offline route: `hearth.wallet.mnemonic` alone leaves `WalletData.accounts` empty (accounts are stored as derivation indices in `wallet.dat`, and nothing has generated any), so the lookup fails with `MissingSenderPrivateKey` rather than deriving the account on demand.
