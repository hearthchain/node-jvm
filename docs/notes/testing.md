...
The `@LoadTest`-annotated suites (`sbt "print node-it/Test/loadTestNames"` for the current list) run in their own CI job (`run-load-tests` in `check-pr.yaml`), through `node-it/loadTests` and pinned to `-Dhearth.it.max-parallel-suites=1`; the main `run-integration-tests` job runs `node-it/integrationTests`, which is the complement. Do not fold them back into the main job. They are two explicit jobs rather than a matrix so the required-check names stay stable, which means the action pins have to be bumped in both.
...
Both halves are selected by `loadTestNames` (`IntegrationTestsPlugin`), which partitions `definedTests` by `isAnnotationPresent` through the test classloader: tagging a suite `@LoadTest` moves it in both directions at once, and there is no tag string in the workflow at all. The annotation is `@Target(ElementType.TYPE)` on purpose - discovery is class-level, so a method-level tag would be excluded from the integration job and never selected by the load job, i.e. silently stop running.

Selecting by name rather than by tag on the command line is the part worth explaining, because the obvious alternative looks like it works. ScalaTest's `-n` (include) *is* honoured on the `test`/`testFull` path: `testFull` with `-n` naming a tag no suite carries runs 0 tests across all 49 suites in 4 seconds, 17 seconds at `max-parallel-suites=1`, starting no containers. What it does not survive is drift. A renamed or mistyped tag in an `-n` argument selects nothing, and sbt reports `No tests were executed` and **exits 0** - green CI running no load tests at all, which is the precise failure this split exists to end. The same drift on the `-l` (exclude) side silently puts the load suites back in the main job. Deriving both halves from one `LoadTestTag` constant makes a rename a `ClassNotFoundException` and an empty selection a `sys.error` instead.

(`-n` is separately bypassed for a suite named on `testOnly`: `Framework.runSuite` rebuilds the filter around `Suite.SELECTED_TAG` when the suite was explicitly specified, so `testOnly X -- -n a.Tag.NothingCarries` runs X in full. That is a property of `testOnly`, not evidence that `-n` is broken - measuring only that path is how this note first got the rule wrong.)
