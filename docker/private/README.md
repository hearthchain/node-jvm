...
docker/private/startboost-live.py <fixture-dir>              # PCK_CRL=processor ... if the quote's PCK leaf is Processor-CA issued
```

Runs on 2026-08-24 (image from this branch, which merges the Reserve/BindApiKey/Settle branch): two fresh containers gave identical layouts: Root CA CRL in block 2, the remaining collateral in 4, commit in 6, the two funding transfers in 8 and 10, StartBoost in 12 (enclave key `4421c67f...` registered for period [1000001, 2000000], operator distinct from the validator), Reserve in 14, BindApiKey in 16, both Settle batches in block 17. The replayed lower counter and the over-reserve counter were both rejected at broadcast, and the operator earned the 30% node share, 240000 embers over the 800000 settled (the script asserts the exact balances). Earlier StartBoost-only runs on 2026-08-21 confirmed the registration path alone in blocks 3-7 on fresh containers.
