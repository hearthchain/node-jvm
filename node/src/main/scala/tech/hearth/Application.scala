...
import tech.hearth.api.http.leasing.LeaseApiRoute
import tech.hearth.api.http.utils.UtilsApiRoute
import tech.hearth.common.state.ByteStr
import tech.hearth.consensus.PoSSelector
import tech.hearth.database.{DBExt, Keys, RDB}
import tech.hearth.events.{BlockchainUpdateTriggers, UtxEvent}
import tech.hearth.extensions.{Context, Extension}
import tech.hearth.features.api.ActivationApiRoute
import tech.hearth.history.{History, StorageFactory}
...
import tech.hearth.utx.{UtxPool, UtxPoolImpl}
...
object Application extends ScorexLogging {
  ...
  private[hearth] def loadBlockAt(rdb: RDB, blockchainUpdater: CompleteBlockchainUpdater)(
      height: Height
  ): Option[(BlockMeta, Seq[(TxMeta, Transaction)])] =
    loadBlockInfoAt(rdb, blockchainUpdater)(height)

  private[hearth] def loadBlockInfoAt(rdb: RDB, blockchainUpdater: CompleteBlockchainUpdater)(
      height: Height
  ): Option[(BlockMeta, Seq[(TxMeta, Transaction)])] =
  ...
  private[hearth] def loadBlockMetaAt(db: RocksDB, blockchainUpdater: CompleteBlockchainUpdater)(height: Height): Option[BlockMeta] =
