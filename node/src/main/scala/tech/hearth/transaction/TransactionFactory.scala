  ...
  def parseRequest(request: JsObject): Either[ValidationError, Transaction & ProvenTransaction] = {
    ...
    try {
      ...
      val req = TransactionType.fromId(typeId) match {
        ...
        case Genesis | Settle | Withdraw =>
          // Settle/Withdraw are not yet signable through this REST flow: their semantics (validation, state diff)
          // are not implemented yet, see TransactionDiffer.
