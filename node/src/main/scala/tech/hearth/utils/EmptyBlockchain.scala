...
import tech.hearth.block.SignedBlockHeader
...
trait EmptyBlockchain extends Blockchain {
  ...
  override def reservedAmount(sender: Address, miner: Address, asset: IssuedAsset): Long = 0L
  ...
  override def stake(address: Address): StakeRecord = StakeRecord.empty

  override def stakers: Seq[Address] = Seq.empty
}
