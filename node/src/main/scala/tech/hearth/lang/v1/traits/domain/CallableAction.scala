...
import tech.hearth.lang.v1.traits.domain.Recipient.{Address, Alias}
...
object Issue {
  ...
  def calculateId(
    ...
    val out = new DataOutputStream(bs)
    out.writeUTF(name)
    ...
    ByteStr(Blake2b256.hash(bs.toByteArray))
  }
}
...
object Lease {
  def calculateId(l: Lease, invokeId: ByteStr): ByteStr = {
    ...
    l.recipient match {
      ...
      case Alias(name)    => out.writeUTF(name)
    }
