...
class MassTransferTransactionDiffTest extends PropSpec with WithDomain {
  ...
  val masterBalance: Seq[AddrWithBalance] = Seq(AddrWithBalance(master.toAddress))
  ...
  property("MassTransfer preserves balance invariant") {
    def testDiff(transferCount: Int): Unit = {
      ...
      val transfer  = TxHelpers.massTransfer(master, transfers, Waves, fee = 1.waves, version = TxVersion.V1)
  ...
  property("MassTransfer cannot overspend funds") {
    ...
    val transfer   = TxHelpers.massTransfer(master, recipients, Waves, version = TxVersion.V1)
