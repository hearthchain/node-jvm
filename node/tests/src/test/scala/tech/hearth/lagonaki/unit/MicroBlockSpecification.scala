...
import tech.hearth.block.{Block, BlockEndorsement, FinalizationVoting, MicroBlock}
...
import tech.hearth.protobuf.block.{PBFinalizationVotings, PBMicroBlocks, SignedMicroBlock}
...
class MicroBlockSpecification extends FunSuite {
  ...
  private val totalResBlockSig = ByteStr(Array.fill(SignatureLength)(Random.nextInt(100).toByte))
  ...
  test("MicroBlock with txs bytes/parse roundtrip, with finalizationVoting") {
    ...
    val blsEndorser     = TxHelpers.blsKeyOf(TxHelpers.signer(10))
    ...
    assert(microBlock.wholeBlockSignature == parsedBlock.wholeBlockSignature)
  ...
  test("MicroBlock serialization matches Go reference output (without signature)") {
    pendingUntilFixed {
      ...
      val microBlock = MicroBlock(
        ...
        wholeBlockSignature = totalResSignature,
