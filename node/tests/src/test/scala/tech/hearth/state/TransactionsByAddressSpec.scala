  ...
  private def test(f: (Address, Seq[Block], Domain) => Unit): Unit = {
    ...
    Seq(recipient1, recipient2).foreach { recipient =>
      withDomain(DomainPresets.RideV6, Seq(AddrWithBalance(sender.toAddress))) { d =>
        ...
        val transactions1 = (1 to txCount1 / 2).flatMap(_ => transfers(sender, recipient.toAddress, Constants.UnitsInHearth))
        val block1        = d.appendBlock(transactions1*)
        val transactions2 = (1 to txCount2 / 2).flatMap(_ => transfers(sender, recipient.toAddress, Constants.UnitsInHearth))
